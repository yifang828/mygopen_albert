...
class LlNclPsX(LtUdnDataset):
    def __init__(self, mode, tokenizer):
        ...
        if self.mode == 'train':
            ...
            self.df = self.df[(self.df['label']=='LL') | (self.df.index.isin(required_ncl_idx)) | \
            ...
            self.df = self.df[(self.df.index.isin(required_test_ll_idx)) | (self.df['label']=='NCL') | \
                (self.df['label']=='PS') | (self.df.index.isin(test_label_x.index))]
            self.df = self.df.drop(['keyWord'], axis=1)
        # required_label
        
        self.len = len(self.df)
        self.label_map = {'LL':0, 'NCL':1, 'PS':2, 'X':3}
